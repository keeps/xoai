...
public class ResumptionToken implements XmlWritable {
  ...
  public static class Value {
    ...
    public boolean isEmpty() {
      return isNull().apply(offset) && isNull().apply(set) && isNull().apply(from) && isNull().apply(until)
    ...
    public Value withScollId(String scrollId) {
      this.scrollId = scrollId;
      return this;
    }

    public Value next(long sum, String scollId) {
      return new Value().withSetSpec(set).withFrom(from).withUntil(until).withMetadataPrefix(metadataPrefix)
        .withOffset((offset == null ? 0 : offset) + sum).withScollId(scollId);
    }
